...
func (suite *AsParserTestSuite) SetupTest() {
...
func (suite *AsParserTestSuite) TestAsInfoRequestInfo() {
	testCases := []struct {
		context      string
	...
	for _, tc := range testCases {
		suite.Run(tc.context, func() {
			// Call GetAsInfo with the input from the test case
			suite.mockConn.EXPECT().RequestInfo([]string{"namespaces", "dcs", "sindex/", "logs", "build"}).Return(tc.coreInfoResp, nil)

			if tc.req != nil {
				suite.mockConn.EXPECT().RequestInfo(tc.req).Return(tc.resp, nil)
			}

			result, err := suite.asinfo.GetAsConfig(tc.context)
...
func (suite *AsParserTestSuite) TestAsInfoRequestInfoXDR5() {
	context := "xdr"
	coreInfoResp := map[string]string{"build": "5.0.0.0"}

	// Call GetAsInfo with the input from the test case
	suite.mockConn.EXPECT().RequestInfo([]string{"namespaces", "dcs", "sindex/", "logs", "build"}).Return(coreInfoResp, nil)
	suite.mockConn.EXPECT().RequestInfo([]string{"get-config:context=xdr"}).Return(map[string]string{"get-config:context=xdr": "dcs=DC1,DC2;src-id=0;trace-sample=0"}, nil)
	suite.mockConn.EXPECT().RequestInfo([]string{"get-config:context=xdr;dc=DC1"}).Return(map[string]string{"get-config:context=xdr;dc=DC1": "auth-mode=none;auth-password-file=null;auth-user=null;connector=false;max-recoveries-interleaved=0;node-address-port=;period-ms=100;tls-name=null;use-alternate-access-address=false;namespaces=test"}, nil)
	...
	result, err := suite.asinfo.GetAsConfig(context)

	suite.Assert().Nil(err)
	suite.Assert().Equal(expected, result)
}

func (suite *AsParserTestSuite) TestAsInfoAllConfigs() {
	// test code here
}

func (suite *AsParserTestSuite) TestAsInfoGetAsInfo() {
	// test code here
}

func (suite *AsParserTestSuite) TestAsInfoGetAsConfig() {
	// test code here
}

func (suite *AsParserTestSuite) TestGetNamespaceNamesCmd() {
	// test code here
}

func (suite *AsParserTestSuite) TestGetDCNamesCmd() {
	// test code here
}

func (suite *AsParserTestSuite) TestGetTLSNamesCmd() {
	// test code here
}

func (suite *AsParserTestSuite) TestGetLogNamesCmd() {
	// test code here
}

func (suite *AsParserTestSuite) TestGetSindexNamesCmd() {
	// test code here
}

func (suite *AsParserTestSuite) TestGetSetNamesCmd() {
	// test code here
}

func (suite *AsParserTestSuite) TestParseNamespaceNames() {
	// test code here
}

func (suite *AsParserTestSuite) TestParseDCNames() {
	// test code here
}

func (suite *AsParserTestSuite) TestParseTLSNames() {
	// test code here
}

func (suite *AsParserTestSuite) TestParseLogNames() {
	// test code here
}

func (suite *AsParserTestSuite) TestParseSindexNames() {
	// test code here
}

func (suite *AsParserTestSuite) TestParseSetNames() {
	// test code here
}
