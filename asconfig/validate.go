...
import (
	...
	internalerrors "github.com/aerospike/aerospike-management-lib/errors"
)
...
func ValidateTLSAuthenticateClient(serviceConf map[string]interface{}) (
	...
	case string:
		if value == keyAny || value == keyFalse {
			return []string{}, nil
		}
...
func validateNamespaceConfig(
	...
	for _, nsConfInterface := range nsConfInterfaceList {
		...
		if nsStorage, ok := nsConf[keyStorageEngine]; ok {
			if nsStorage == nil {
				return fmt.Errorf(
					"%s cannot be nil for namespace %v", keyStorageEngine, nsConf,
				)
			}
			...
			if !IsDeviceOrPmemNamespace(nsConf) {
				return fmt.Errorf(
					"%s not supported for namespace %v", keyStorageEngine, nsConf,
				)
			}
...
func IsSecurityEnabled(config map[string]interface{}) (bool, error) {
	if _, err := GetConfigContext(config, "security"); err != nil {
...
func ValidateAerospikeConfigSchema(
	...
	asConf, err := NewMapAsConfig(aslog, config)
	if err != nil {
...
func GetConfigContext(
	config map[string]interface{}, context string,
) (map[string]interface{}, error) {
	if config == nil {
		return nil, fmt.Errorf("aerospike configuration is nil")
	}

	if contextConfigMap, ok := config[context]; ok {
