...
// 	passwordProvider AerospikeUserPasswordProvider, enabled bool,
...
// 		return "", "", fmt.Errorf(
// 			"%s user missing in access control", AdminUsername,
// 		)
// 	}

// 	password, err := passwordProvider.Get(
// 		AdminUsername, &adminUserSpec,
// 	)
// 	if err != nil {
// 		return "", "", err
// 	}
...
func ReconcileAccessControl(
	...
	currentRoles := GetRolesFromSpec(currentState)

	if err := reconcileRoles(
		desiredRoles, currentRoles, client, adminPolicy, logger,
	); err != nil {
		return err
	}

	desiredUsers := GetUsersFromSpec(desired)
	currentUsers := GetUsersFromSpec(currentState)

	return reconcileUsers(
		desiredUsers, currentUsers, passwordProvider, client, adminPolicy, logger,
	)
}

// GetAdminPolicy returns the AdminPolicy to use for performing access control operations.
func GetAdminPolicy(accessControlSpec *AerospikeAccessControlSpec) as.AdminPolicy {
	if accessControlSpec == nil || accessControlSpec.AdminPolicy == nil {
		return *as.NewAdminPolicy()
	}

	specAdminPolicy := *accessControlSpec.AdminPolicy
...
func reconcileRoles(
	desired map[string]AerospikeRoleSpec,
	current map[string]AerospikeRoleSpec, client *as.Client,
	...
	for _, roleToDrop := range rolesToDrop {
		if _, ok := PredefinedRoles[roleToDrop]; !ok {
	...
	for _, cmd := range roleReconcileCmds {
		if err := cmd.execute(client, &adminPolicy, logger); err != nil {
			return err
		}
	}
...
func reconcileUsers(
	desired map[string]AerospikeUserSpec,
	current map[string]AerospikeUserSpec,
...
type AerospikeUserPasswordProvider interface {
	...
	Get(username string, userSpec *AerospikeUserSpec) (
		string, error,
	)
}
...
func (roleCreate aerospikeRoleCreateUpdate) execute(
	...
	if isCreate {
		if err := roleCreate.createRole(client, adminPolicy, logger); err != nil {
			// recorder.Eventf(
...
func (roleCreate aerospikeRoleCreateUpdate) updateRole(
	client *as.Client, adminPolicy *as.AdminPolicy, role *as.Role, logger logger,
	// recorder record.EventRecorder, aeroCluster *asdbv1.AerospikeCluster,
) error {
	// Update the role.
...
func (userCreate aerospikeUserCreateUpdate) execute(
	...
	if isCreate {
		err := userCreate.createUser(client, adminPolicy, logger)
		if err != nil {
			// recorder.Eventf(
...
func (userCreate aerospikeUserCreateUpdate) updateUser(
	client *as.Client, adminPolicy *as.AdminPolicy, user *as.UserRoles, logger logger,
	// recorder record.EventRecorder, aeroCluster *asdbv1.AerospikeCluster,
) error {
	// Update the user.
